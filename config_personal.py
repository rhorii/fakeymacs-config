...
fc.transparent_target  += ["VirtualBoxVM.exe"]
...
fc.skip_settings_key    = {"keymap_base"      : ["*W-g"],                 # ベース Keymap
                           ...
                           "keymap_emacs"     : ["C-Space", "Ctl-x C-c"], # Emacs キーバインド対象アプリ用 Keymap
...
fc.use_ctrl_openbracket_as_meta = False
...
## HHKB用
fc.set_input_method_key += [["O-RCtrl", "O-RWin"]]
...
fc.desktop_switching_key += [["LCtrl-Left", "LCtrl-Right"]]
...
####################################################################################################
# スタートメニュー
define_key(keymap_global, "RCtrl-Space", self_insert_command("RWin"))

# タスクビュー
define_key(keymap_global, "LCtrl-Up", self_insert_command("W-Tab"))

# RWin キーを単押しした際に、スタートメニューがされないようにするための対策
define_key(keymap_global, "D-RWin", self_insert_command("Ctrl"))
